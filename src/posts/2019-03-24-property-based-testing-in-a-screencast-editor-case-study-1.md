...
series](/2019-03-02-property-based-testing-in-a-screencast-editor-introduction.html)
...
![Clips and gaps are placed in video and audio tracks](/assets/pbt-screencast-editor/diagram1.png){width=350}
...
![Still frames are automatically inserted at implicit gaps to match track duration](/assets/pbt-screencast-editor/diagram2.png){width=350}
...
![Adding explicit gaps manually](/assets/pbt-screencast-editor/diagram3.png){width=350}
...
![A sequence containing two parallels](/assets/pbt-screencast-editor/diagram4.png){width=600}
...
![A timeline containing two sequences, with two parallels each](/assets/pbt-screencast-editor/diagram5.png){width=750}
...
![Timeline flattening transforming a hierarchical timeline](/assets/pbt-screencast-editor/diagram6.png){width=750}
...
hprop_flat_timeline_uses_still_frame_from_single_clip =
  property $ do
    -- 1. Generate a video track generator where the first
    -- video part is always a clip
    let genVideoTrack = do
          v1 <- Gen.videoClip
          vs <- Gen.list (Range.linear 1 5) Gen.videoGap
          pure (VideoTrack () (v1 : vs))

    -- 2. Generate a timeline with the custom video track
    -- generator
    ...
    -- 4. Check that any video gaps will use the last frame
    -- of a preceding video clip
    flat
      ^.. ( _Just
          . Render.videoParts
          . each
...
![Still frames being sourced from the single initial video clip](/assets/pbt-screencast-editor/diagram7.png){width=375}
...
hprop_flat_timeline_uses_still_frames_from_subsequent_clips =
  property $ do
    -- 1. Generate a parallel where the video track ends with
    -- a video clip, and where the audio track is shorter
    let
      genParallel = do
        vt <-
          VideoTrack ()
...
![Still frames being sourced from following video clips when possible](/assets/pbt-screencast-editor/diagram8.png){width=600}
...
![Still frames being sourced from preceding video clip for last implicit gap](/assets/pbt-screencast-editor/diagram9.png){width=350}
...
hprop_flat_timeline_is_same_as_all_its_flat_sequences =
  property $ do
    ...
    -- 4. Flatten the entire timeline and compare to the
    -- flattened sequences
