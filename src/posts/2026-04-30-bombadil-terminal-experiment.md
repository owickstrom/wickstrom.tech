...
user interfaces. One that I already wanted to explore is *terminal user
...
2. [libghostty-vt](https://crates.io/crates/libghostty-vt), which interprets
   the output of the PTY and provides a virtual terminal API from which you can
   read cell contents, styles, scroll through the scrollback, etc.
...
sequences (printable ASCII characters and ANSI escape sequences). Timing is a
bit tricky, but it seems the current approach works fine: polling reads until
the terminal is idle, capture state, then apply new inputs.
...
* `vitetris` has a bug where if you enter just a number in the host name (e.g.
  `6`) and try to connect to a remote game, the UI freezes.
* `btop` has two different bugs, [one recently
...
Today, I merged this to `main` in Bombadil. It's not yet released, but if
you're curious you can try it already by downloading a `bombadil-terminal`
binary from the
...
* Make the test runner user interface better. Perhaps a TUI?
