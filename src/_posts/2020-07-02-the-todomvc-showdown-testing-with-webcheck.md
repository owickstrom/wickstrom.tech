...
[TodoMVC](http://todomvc.com/) examples using my new testing tool named
_WebCheck_. I'll explain how the specification works, what problems were
found in the TodoMVC implementations, and what this means for my project.
...
the _temporal logic of actions_ from TLA+. WebCheck generates hundreds or
thousands of tests and runs them to verify if your application is accepted by
the specification.
...
application with a myriad of seemingly arbitrary rules, the model tends to
grow _as complex_ to be useful.
...
be more than one spinner one a page", and nothing else.

### TodoMVC as a Benchmark

Since the start of this project, I've used the TodoMVC examples as a
benchmark of WebCheck, and developed a general specification for TodoMVC
applications. The TodoMVC contribution docs has [a high-level feature
