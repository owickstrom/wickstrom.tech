Adipisicing earum atque maiores expedita labore. Consequuntur dolor itaque aut
quaerat dignissimos provident saepe deserunt dicta ipsum magnam! Nostrum
voluptatem ipsum tempore aspernatur obcaecati labore! Blanditiis quae dolor
eveniet perferendis.

Data Structure Representation
-----------------------------

Sit aliquid rerum ratione quaerat repellendus illum reiciendis quae quis
obcaecati. Numquam alias quae culpa aspernatur dicta. Consequatur facere dolore
doloremque similique quidem esse repellat quaerat sunt nobis sunt eveniet?

Finally Tagless Encoding
------------------------

As this article is compiled from _Literate Haskell source code_, we
...
The core language is expressed in *final encoding*, meaning that all
operations are overloaded functions in a type class. `MonadSpec` takes
two type arguments; a Monad `m`, and an Applicative `f`, and includes
the operations `it`, `describe`, `beforeEach`, and `beforeAll`.
...
essential for our operations to compose. For now, though, it is enough to think
of `it` as way of constructing a `Spec` from a single test.
